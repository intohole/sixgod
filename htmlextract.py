...
class HtmlExtract(object):
    ...
    pre_reg_list = [re.compile(
        '(?is)<!DOCTYPE.*?>', re.I), re.compile("(?is)<!--.*?-->", re.I), re.compile('(?is)<script.*?>.*?</script>', re.I),
        re.compile("(?is)<style.*?>.*?</style>", re.I), re.compile("&.{2,5};|&#.{2,5};", re.I), re.compile('<!--.*?>'), re.compile("(?is)<.*?>", re.I)]
    ...
    def get_text(self, html):
        text_line = [i for i in self.pre_process(
...
h = HtmlExtract()
html = util.get_url_string(
    "http://news.xinhuanet.com/world/2013-10/09/c_125497473.htm")
try:
